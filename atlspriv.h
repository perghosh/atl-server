...
{
public:
	ZEvtSyncSocket() noexcept;
	~ZEvtSyncSocket() noexcept;
	operator SOCKET() noexcept;
	void Close() noexcept;
	void Term() noexcept;
	bool Create(const ADDRINFOT* pAI, WORD wFlags=0) noexcept;
	bool Create(int af, int st, int proto, WORD wFlags=0) noexcept;
	bool Connect(LPCTSTR szAddr, unsigned short nPort) noexcept;
	bool Connect(const SOCKADDR* psa, int len) noexcept;
	bool Connect(const ADDRINFOT *pAI) noexcept;
	bool Write(WSABUF *pBuffers, int nCount, DWORD *pdwSize) noexcept;
	bool Write(const unsigned char *pBuffIn, DWORD *pdwSize) noexcept;
	bool Read(const unsigned char *pBuff, DWORD *pdwSize) noexcept;
	bool Init(SOCKET hSocket, void * /*pData=NULL*/) noexcept;
	DWORD GetSocketTimeout() noexcept;
	DWORD SetSocketTimeout(DWORD dwNewTimeout) noexcept;
	bool SupportsScheme(ATL_URL_SCHEME scheme) noexcept;
...
inline BOOL AtlMimeCharsetFromCodePage(_Out_z_cap_(cch) LPSTR szCharset, _In_ UINT uiCodePage, _In_opt_ IMultiLanguage* pMultiLanguage, _In_ size_t cch) noexcept
{
...
public:
	CStreamOnSequentialStream(ISequentialStream *pStream) noexcept
	{
		ATLASSERT(pStream);
		m_spStream = pStream;
	}
	...
	STDMETHOD(Write)(const void *pv, ULONG cb, ULONG *pcbWritten) noexcept
	{
		if (!m_spStream)
			return E_UNEXPECTED;
	...
	STDMETHOD(Seek)(LARGE_INTEGER , DWORD , ULARGE_INTEGER *) noexcept
	{
		return E_NOTIMPL;
	}

	STDMETHOD(SetSize)(ULARGE_INTEGER ) noexcept
	{
		return E_NOTIMPL;
	}

	STDMETHOD(CopyTo)(IStream *, ULARGE_INTEGER , ULARGE_INTEGER *,
		ULARGE_INTEGER *) noexcept
	{
		return E_NOTIMPL;
	}

	STDMETHOD(Commit)(DWORD ) noexcept
	{
		return E_NOTIMPL;
	}
	...
	STDMETHOD(Stat)(STATSTG *, DWORD ) noexcept
	{
		return E_NOTIMPL;
	}

	STDMETHOD(Clone)(IStream **) noexcept
	{
		return E_NOTIMPL;
	}
...
public:
	...
	CStreamOnByteArray(BYTE *pBytes) noexcept
	{
		ATLASSERT(pBytes);
		m_pArray = pBytes;
		m_dwRead = 0;
...
public:
	...
	HRESULT InsertVariant(const VARIANT *pVarIn) noexcept
	{
		CComVariant vIn;
		HRESULT hr = E_FAIL;
		m_nCurrRead = 0;
	...
	// out variant.
	HRESULT RetrieveVariant(VARIANT *pVarOut) noexcept
	{
		CComVariant vOut;
	...
	HRESULT LoadFromStream(ISequentialStream *stream) noexcept
	{
		m_nCurrRead = 0;
	...
	ISequentialStream* GetStream() noexcept
	{
		return static_cast<ISequentialStream*>(this);
	}

	size_t GetVariantSize() noexcept
	{
