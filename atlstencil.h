...
public:
	...
	bool RenderErrors(IWriteStream *pStream)
	{
		if (pStream == NULL)
		{
			return false;
	...
	virtual bool ParseReplacements(ITagReplacer* pReplacer)
	{
		return ParseReplacementsFromBuffer(pReplacer, GetBufferStart(), GetBufferEnd());
	}

	virtual bool FinishParseReplacements()
	{
	...
	virtual bool Parse(ITagReplacer *pReplacer)
	{
	...
	virtual HTTP_CODE Render(
		...
		CStencilState* pState = NULL) const
	{
...
struct CStringPair
{
	...
	CStringPair(PathStrType &strDllPath_, HdlrNameStrType &strHandlerName_)
...
public:
	...
	void Initialize(IServiceProvider *pProvider)
	{
		ATLENSURE(pProvider);
...
#pragma warning(pop)
	...
	DWORD ParseInclude(LPCSTR szTokenStart, LPCSTR szTokenEnd)
	{
...
public:
	...
	HTTP_CODE Initialize(AtlServerRequest *pRequestInfo, IHttpServerContext *pSafeSrvCtx=NULL)
	{
	...
	HTTP_CODE LoadStencilResource(
		...
		LPCSTR szResourceID,
		LPCSTR szResourceType = NULL, LPCSTR szStencilName=NULL)
	{
		if (!szResourceType)
	...
	HTTP_CODE LoadStencilResource(HINSTANCE hInstResource, UINT nID, LPCSTR szResourceType = NULL)
	{
		if (!szResourceType)
			szResourceType = (LPCSTR) RT_HTML;
		char szName[80];
	...
	HTTP_CODE LoadStencil(LPCSTR szFileName, IHttpRequestLookup * pLookup = NULL)
	{
		if (!szFileName)
		{
			return HTTP_FAIL;
	...
	HTTP_CODE RenderStencil(IWriteStream* pStream, CStencilState* pState = NULL)
	{
		if (!m_pLoadedStencil)
			return AtlsHttpError(500, ISE_SUBERR_UNEXPECTED);
...
//Implementation
	...
	StencilType* GetNewCacheStencil()
	{
	...
	HTTP_CODE FinishLoadStencil(StencilType *pStencil, IHttpRequestLookup * pLookup = NULL)
	{
		THandler *pT = static_cast<THandler *>(this);
		ATLASSERT(pStencil);
		if (!pStencil)
