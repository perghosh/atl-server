...
inline ZEvtSyncSocket::ZEvtSyncSocket() noexcept
{
...
inline ZEvtSyncSocket::operator SOCKET()  noexcept
{
	return m_socket;
}

inline void ZEvtSyncSocket::Close()  noexcept
{
...
inline void ZEvtSyncSocket::Term() noexcept 
{
	if (m_hEventRead)
	{
...
inline bool ZEvtSyncSocket::Create(const ADDRINFOT* pAI, WORD wFlags) noexcept
{
	return Create(pAI->ai_family, pAI->ai_socktype, pAI->ai_protocol, wFlags);
}

inline bool ZEvtSyncSocket::Create(int af, int st, int proto, WORD wFlags)  noexcept
{
	bool bRet = true;
	if (m_socket != INVALID_SOCKET)
	{
...
inline bool ZEvtSyncSocket::Connect(const ADDRINFOT *pAI) noexcept
{
...
inline bool ZEvtSyncSocket::Connect(const SOCKADDR* psa, int len) noexcept 
{
	if (m_bConnected)
...
inline bool ZEvtSyncSocket::Write(WSABUF *pBuffers, int nCount, DWORD *pdwSize) noexcept 
{
...
inline bool ZEvtSyncSocket::Write(const unsigned char *pBuffIn, DWORD *pdwSize) noexcept 
{
	WSABUF buff;
...
inline bool ZEvtSyncSocket::Read(const unsigned char *pBuff, DWORD *pdwSize) noexcept 
{
	// if we aren't already connected we'll wait to see if the connect
	// event happens
	if (WAIT_OBJECT_0 != WaitForSingleObject((HANDLE)m_hEventConnect , m_dwSocketTimeout))
...
inline bool ZEvtSyncSocket::Init(SOCKET hSocket, void * /*pData=NULL*/) noexcept 
{
