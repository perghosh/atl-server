...
public:
	...
	CSessionNameGenerator() noexcept :
	...
	HRESULT GetNewSessionName(__out_ecount_part_z(*pdwSize, *pdwSize) LPSTR szNewID, __inout DWORD *pdwSize) noexcept
	{
		HRESULT hr = E_FAIL;
	...
	DWORD CalcMaxInputSize(DWORD nOutputSize) noexcept
	{
	...
	HRESULT GenerateRandomName(BYTE *pBuff, DWORD dwBuffSize) noexcept
	{
		if (!pBuff)
			return E_POINTER;
...
{
public:
	LPCTSTR GetSessionRefDelete() noexcept
	{
	...
	LPCTSTR GetSessionRefIsExpired() noexcept
	{
	...
	LPCTSTR GetSessionRefDeleteFinal() noexcept
	{
		return _T("DELETE FROM SessionReferences ")
			   _T("WHERE SessionID=?");
	}

	LPCTSTR GetSessionRefCreate() noexcept
	{
	...
	LPCTSTR GetSessionRefUpdateTimeout() noexcept
	{
		return _T("UPDATE SessionReferences ")
			   _T("SET TimeoutMs=? WHERE SessionID=?");
	}

	LPCTSTR GetSessionRefAddRef() noexcept
	{
		return _T("UPDATE SessionReferences ")
	...
	LPCTSTR GetSessionRefRemoveRef() noexcept
	{
		return _T("UPDATE SessionReferences ")
					_T("SET RefCount=RefCount-1, ")
					_T("LastAccess=getdate() ")
	...
	LPCTSTR GetSessionRefAccess() noexcept
	{
		return 	_T("UPDATE SessionReferences ")
	...
	LPCTSTR GetSessionRefSelect() noexcept
	{
		return _T("SELECT * FROM SessionReferences ")
			   _T("WHERE SessionID=?");
	}

	LPCTSTR GetSessionRefGetCount() noexcept
	{
		return 	_T("SELECT COUNT(*) FROM SessionReferences");
	}


	LPCTSTR GetSessionVarCount() noexcept
	{
		return _T("SELECT COUNT(*) FROM SessionVariables WHERE SessionID=?");
	}

	LPCTSTR GetSessionVarInsert() noexcept
	{
	...
	LPCTSTR GetSessionVarUpdate() noexcept
	{
	...
	LPCTSTR GetSessionVarDeleteVar() noexcept
	{
		return _T("DELETE FROM SessionVariables ")
				_T("WHERE SessionID=? AND VariableName=?");
	}

	LPCTSTR GetSessionVarDeleteAllVars() noexcept
	{
	...
	LPCTSTR GetSessionVarSelectAllVars() noexcept
	{
		return _T("SELECT SessionID, VariableName, VariableValue ")
				_T("FROM SessionVariables ")
				_T("WHERE SessionID=?");
	...
	LPCTSTR GetSessionReferencesSet() noexcept
	{
...
public:
	...
	HRESULT Assign(LPCTSTR szSessionID, LPCTSTR szVarName, VARIANT *pVal) noexcept
	{
		HRESULT hr = S_OK;
...
public:
	...
	TCHAR m_szSessionID[MAX_SESSION_KEY_LEN];
	TCHAR m_VariableName[MAX_VARIABLE_NAME_LENGTH];
	HRESULT Assign(LPCTSTR szSessionID, LPCTSTR szVarName) noexcept
	{
		if (szSessionID)
		{
			if (Checked::tcsnlen(szSessionID, MAX_SESSION_KEY_LEN) < MAX_SESSION_KEY_LEN)
...
public:
	TCHAR m_szSessionID[MAX_SESSION_KEY_LEN];
	HRESULT Assign(LPCTSTR szSessionID) noexcept
	{
		if (!szSessionID)
			return E_INVALIDARG;
...
public:
	...
	HRESULT Assign(LPCTSTR szSessionID, unsigned __int64 nNewTimeout) noexcept
	{
		if (!szSessionID)
			return E_INVALIDARG;
...
public:
	...
	HRESULT Assign(LPCTSTR szSessionID, unsigned __int64 timeout) noexcept
	{
		if (!szSessionID)
			return E_INVALIDARG;
		if (Checked::tcsnlen(szSessionID, MAX_SESSION_KEY_LEN) < MAX_SESSION_KEY_LEN)
...
public:
	...
	STDMETHOD(SetVariable)(LPCSTR szName, VARIANT Val) noexcept
	{
		HRESULT hr = E_FAIL;
		if (!szName)
			return E_INVALIDARG;
	...
	STDMETHOD(GetVariable)(LPCSTR szName, VARIANT *pVal) noexcept
	{
		HRESULT hr = E_FAIL;
		if (!szName)
			return E_INVALIDARG;
	...
	STDMETHOD(RemoveVariable)(LPCSTR szName) noexcept
	{
		HRESULT hr = E_FAIL;
		if (!szName)
			return E_INVALIDARG;
	...
	STDMETHOD(GetCount)(long *pnCount) noexcept
	{
		HRESULT hr = S_OK;
		if (pnCount)
			*pnCount = 0;
	...
	STDMETHOD(RemoveAllVariables)() noexcept
	{
	...
	STDMETHOD(BeginVariableEnum)(POSITION *pPOS, HSESSIONENUM *phEnum) noexcept
	{
		HRESULT hr = E_FAIL;
		if (!pPOS)
			return E_POINTER;
	...
	// variable
	STDMETHOD(GetNextVariable)(POSITION *pPOS, VARIANT *pVal, HSESSIONENUM hEnum, LPSTR szName=NULL, DWORD dwLen=0) noexcept
	{
		if (!pPOS)
			return E_INVALIDARG;
	...
	STDMETHOD(CloseEnum)(HSESSIONENUM hEnum) noexcept
	{
		iterator_accessor *pIteratorAccessor = reinterpret_cast<iterator_accessor*>(hEnum);
		if (!pIteratorAccessor)
			return E_INVALIDARG;
	...
	STDMETHOD(SetTimeout)(unsigned __int64 dwNewTimeout) noexcept
	{
		HRESULT hr = E_UNEXPECTED;
	...
	// refcount is 0
	HRESULT SessionUnlock() noexcept
	{
		HRESULT hr = E_UNEXPECTED;
		if (!m_szSessionName ||
			m_szSessionName[0]==0)
	...
	HRESULT Initialize( LPCSTR szSessionName, 
						...
						DWORD_PTR dwCookie,
						PFN_GETPROVIDERINFO pfnInfo) noexcept
	{
		if (!szSessionName)
			return E_INVALIDARG;
	...
	HRESULT GetSessionConnection(CDataConnection *pConn,
								 IServiceProvider *pProv) noexcept
	{
		if (!pProv)
			return E_INVALIDARG;
...
public:
	...
	CDBSessionServiceImplT() noexcept
	{
		m_dwTimeout = ATL_SESSION_TIMEOUT;
		m_szConnectionString[0] = '\0';
	}

	static bool GetProviderInfo(DWORD_PTR dwProvCookie, wchar_t **ppszProvInfo) noexcept
	{
		if (dwProvCookie &&
			ppszProvInfo)
		{
	...
	HRESULT Initialize(SERVICEIMPL_INITPARAM_TYPE pData,
						...
						unsigned __int64 dwInitialTimeout) noexcept
	{
		if (!pData || !pProvider)
			return E_INVALIDARG;
	...
	HRESULT CreateNewSession(__out_ecount_part_z(*pdwSize, *pdwSize) LPSTR szNewID, __inout DWORD *pdwSize, __deref_out ISession** ppSession) noexcept
	{
		HRESULT hr = E_FAIL;
	...
	HRESULT CreateNewSessionByName(__in_z LPSTR szNewID, __deref_out ISession** ppSession) noexcept
	{
		HRESULT hr = E_FAIL;
		CComObject<TDBSession> *pNewSession = NULL;
	...
	HRESULT GetSession(LPCSTR szID, ISession **ppSession) noexcept
	{
		HRESULT hr = E_FAIL;
		if (!szID)
			return E_INVALIDARG;
	...
	HRESULT CloseSession(LPCSTR szID) noexcept
	{
		if (!szID)
			return E_INVALIDARG;
	...
	HRESULT SetSessionTimeout(unsigned __int64 nTimeout) noexcept
	{
		// Get the data connection for this thread
		CDataConnection conn;
	...
	HRESULT GetSessionTimeout(unsigned __int64* pnTimeout) noexcept
	{
		if (pnTimeout)
	...
	HRESULT GetSessionCount(DWORD *pnCount) noexcept
	{
		if (pnCount)
			*pnCount = 0;
		else
	...
	void ReleaseAllSessions() noexcept
	{
		// nothing to do
	}

	void SweepSessions() noexcept
	{
		// nothing to do
	}
	...
	// Helpers
	HRESULT IsValidSession(LPCTSTR szID) noexcept
	{
		if (!szID)
			return E_INVALIDARG;
...
{
public:
	static ULONG Hash( INARGTYPE obj ) noexcept
	{
		return( (ULONG)(ULONG_PTR)obj.p);
	}

	static BOOL CompareElements( OUTARGTYPE element1, OUTARGTYPE element2 ) noexcept
	{
		return element1.IsEqualObject(element2.p) ? TRUE : FALSE;
	}

	static int CompareElementsOrdered( INARGTYPE , INARGTYPE ) noexcept
	{
...
public:
	...
	STDMETHOD(SetVariable)(LPCSTR szName, VARIANT vNewVal) noexcept
	{
		if (!szName)
			return E_INVALIDARG;
	...
	STDMETHOD(BeginVariableEnum)(POSITION *pPOS, HSESSIONENUM *phEnumHandle=NULL) noexcept
	{
		if (phEnumHandle)
			*phEnumHandle = NULL;
	...
	STDMETHOD(CloseEnum)(HSESSIONENUM /*hEnumHandle*/) noexcept
	{
		return S_OK;
	}
...
public:
	...
	CMemSessionServiceImpl() noexcept
	{
		m_dwTimeout = ATL_SESSION_TIMEOUT;
	}
	...
	HRESULT CreateNewSession(__out_ecount_part_z(*pdwSize, *pdwSize) LPSTR szNewID, __inout DWORD *pdwSize, __deref_out_opt ISession** ppSession) noexcept
	{
		HRESULT hr = E_FAIL;
	...
	HRESULT CreateNewSessionByName(__in_z LPSTR szNewID, __deref_out_opt ISession** ppSession) noexcept
	{
		HRESULT hr = E_FAIL;
		CComObject<CMemSession> *pNewSession = NULL;
	...
	HRESULT Initialize(SERVICEIMPL_INITPARAM_TYPE,
					   IServiceProvider*,
					   unsigned __int64 dwNewTimeout) noexcept
	{
		m_dwTimeout = dwNewTimeout;
...
// ISessionStateServie methods
	STDMETHOD(CreateNewSession)(LPSTR szNewID, DWORD *pdwSize, ISession** ppSession) noexcept
	{
		return m_SessionServiceImpl.CreateNewSession(szNewID, pdwSize, ppSession);
	}

	STDMETHOD(CreateNewSessionByName)(LPSTR szNewID, ISession** ppSession) noexcept
	{
		return m_SessionServiceImpl.CreateNewSessionByName(szNewID, ppSession);
	}

	STDMETHOD(GetSession)(LPCSTR szID, ISession **ppSession) noexcept
	{
		return m_SessionServiceImpl.GetSession(szID, ppSession);
	}

	STDMETHOD(CloseSession)(LPCSTR szSessionID) noexcept
	{
		return m_SessionServiceImpl.CloseSession(szSessionID);
	}

	STDMETHOD(SetSessionTimeout)(unsigned __int64 nTimeout) noexcept
	{
		return m_SessionServiceImpl.SetSessionTimeout(nTimeout);
	}

	STDMETHOD(GetSessionTimeout)(unsigned __int64 *pnTimeout) noexcept
	{
		return m_SessionServiceImpl.GetSessionTimeout(pnTimeout);
	}

	STDMETHOD(GetSessionCount)(DWORD *pnSessionCount) noexcept
	{
	...
	HRESULT Initialize(
		...
		unsigned __int64 dwTimeout = ATL_SESSION_TIMEOUT) noexcept
	{
		HRESULT hr = S_OK;
		if (pServiceProvider)
			m_spServiceProvider = pServiceProvider;
...
// Implementation
	HRESULT Execute(DWORD_PTR /*dwParam*/, HANDLE /*hObject*/) noexcept
	{
		SweepSessions();
		return S_OK;
	}

	HRESULT CloseHandle(HANDLE hHandle) noexcept
	{
		::CloseHandle(hHandle);
		m_hTimer = NULL;
		return S_OK;
