...
// template <>
// inline bool IsNullByType<long>(long type) noexcept
// {
//   return type == 0;
// }
...
inline bool IsNullByType(__in TComp type) noexcept
{
	return type == 0;
}
...
inline bool EscapeToCString(__inout CStringA& string, __in_z LPCWSTR wszBuf) noexcept
{
	_ATLTRY
	{
...
struct CDefaultErrorProvider
{
	...
	//          puResId is optional
	static BOOL GetErrorText(__in UINT uError, __in UINT uSubErr, __deref_out_opt LPCSTR *ppszHeader, __out_opt UINT *puResId) noexcept
	{
...
void GetStatusHeader(__inout CStringA &strStatus, __in DWORD dwStatus, __in DWORD dwSubStatus, __in HttpUserErrorTextProvider* pErrorProvider, __out_opt UINT *puResId = NULL)
{	
...
public:
	...
	CIsapiWorker() noexcept
	{
		m_hHeap = NULL;
	}

	virtual ~CIsapiWorker() noexcept
	{
	...
	virtual void Terminate(__inout_opt __crt_typefix(IIsapiExtension*) void* pvParam) noexcept
	{
		if (m_hHeap)
		{
	...
	virtual BOOL GetWorkerData(DWORD /*dwParam*/, void ** /*ppvData*/) noexcept
	{
		return FALSE;
	}
};
...
public:
	...
	CWrappedServerContext() noexcept
	{
	}

	virtual ~CWrappedServerContext() noexcept
	{
	}

	CWrappedServerContext(__in IHttpServerContext *pParent) noexcept
	{
		m_spParent = pParent;
	}
...
public:
	...
	CServerContext() noexcept
	{
		m_pECB = NULL;
		m_bHeadersHaveBeenSent = false;
	}
	virtual ~CServerContext() noexcept
	{
	}
...
public:
	...
	static BOOL Add(__inout PeerInfo * pDest, __in PeerInfo * pSrc) noexcept
	{
		_ATLTRY
		{
	...
	static BOOL Remove(const PeerInfo * /*pDest*/) noexcept
	{
		return TRUE;
	}
};
...
public:
	...
	CCacheServerContext() noexcept
	{
	}
	virtual ~CCacheServerContext() noexcept
	{
	}

	BOOL Initialize(__in IHttpServerContext *pParent, __in IFileCache *pCache) noexcept
	{
		ATLASSERT(pParent);
		ATLASSERT(pCache);
	...
	HRESULT GetExpiration(__out FILETIME *pftExpiration) noexcept
	{
	...
	HRESULT SetExpiration(__in FILETIME ftExpiration) noexcept
	{
	...
	__checkReturn BOOL IsCached() noexcept
	{
		return m_bIsCached;
	}

	BOOL Cache(__in BOOL bCache) noexcept
	{
...
public:
	...
	CValidateContext(__in DWORD dwFlags=0) noexcept
	{
		m_bFailures = false;
		m_dwFlags = dwFlags;
	}
	...
	bool AddResult(__in LPCSTR szName, __in DWORD type, __in bool bOnlyFailures = true) noexcept
	{
		_ATLTRY
		{
			if (!VALIDATION_SUCCEEDED(type) ||
	...
	__checkReturn bool ParamsOK() noexcept
	{
		return !m_bFailures;
	}

	// Returns the number of validation results in the collection.
	__checkReturn int GetResultCount() noexcept
	{
	...
	__checkReturn bool GetResultAt(__in int i, __out CStringA& strName, __out DWORD& type) noexcept
	{
...
public:
	...
	static DWORD Validate(
		...
		__in TCompType nMaxValue) noexcept
	{
	...
	static DWORD Validate( __in LPCSTR pszValue, __in int nMinChars, __in int nMaxChars) noexcept
	{
		DWORD dwRet = VALIDATION_S_OK;
		if(!pszValue)
		{
	...
	static DWORD Validate( __in double dblValue, __in double dblMinValue, __in double dblMaxValue) noexcept
	{
		DWORD dwRet = VALIDATION_S_OK;
...
public:
	...
	CCookie(__in LPCSTR szName)
	{
		ATLENSURE(SetName(szName));
	}

	// Constructs a single-valued cookie.
	CCookie(__in LPCSTR szName, __in_opt LPCSTR szValue)
	{
		ATLENSURE(SetName(szName));
		ATLENSURE(SetValue(szValue));
	}

	CCookie(__in const CCookie& thatCookie)
	{
		Copy(thatCookie);
	}

	CCookie& operator=(__in const CCookie& thatCookie)
	{
		if(this!=&thatCookie)
		{
	...
	__checkReturn BOOL SetName(__in LPCSTR szName) noexcept
	{
		_ATLTRY
		{
			if (szName && *szName)
	...
	__checkReturn BOOL SetValue(__in_opt LPCSTR szValue) noexcept
	{
		_ATLTRY
		{
	...
	__checkReturn BOOL AddValue(__in LPCSTR szName, __in_opt LPCSTR szValue) noexcept
	{
	...
	__checkReturn BOOL ModifyValue(__in LPCSTR szName, __in LPCSTR szValue) noexcept
	{
	...
	// Returns TRUE on success, FALSE on failure.
	__checkReturn BOOL RemoveValue(__in LPCSTR szName) noexcept
	{
	...
	__checkReturn BOOL AddAttribute(__in LPCSTR szName, __in LPCSTR szValue) noexcept
	{
	...
	// Both functions will add the attribute if not already present or
	// change its value if it has already been applied to the cookie.
	__checkReturn BOOL ModifyAttribute(__in LPCSTR szName, __in LPCSTR szValue) noexcept
	{
	...
	// Returns TRUE on success, FALSE on failure.
	__checkReturn BOOL RemoveAttribute(__in LPCSTR szName) noexcept
	{
	...
	// Version 1 attribute.
	__checkReturn BOOL SetComment(__in LPCSTR szComment) noexcept
	{
	...
	// This attribute is optional.
	// Version 1 attribute.
	__checkReturn BOOL SetCommentUrl(__in LPCSTR szUrl) noexcept
	{
		BOOL bRet = SetVersion(1);
		if (bRet)
	...
	// Version 1 attribute.
	__checkReturn BOOL SetDiscard(__in BOOL bDiscard) noexcept
	{
		BOOL bRet = FALSE;
	...
	__checkReturn BOOL SetDomain(__in LPCSTR szDomain) noexcept
	{
		BOOL bRet = SetVersion(1);
		if (bRet)
	...
	// Version 1 attribute.
	__checkReturn BOOL SetMaxAge(__in UINT nMaxAge) noexcept
	{
		BOOL bRet = FALSE;
		bRet = SetVersion(1);
		if (bRet)
	...
	// Version 0 & Version 1 attribute.
	__checkReturn BOOL SetPath(__in LPCSTR szPath) noexcept
	{   
		BOOL bRet = SetVersion(1);
		if (bRet)
	...
	// Version 1 attribute.
	__checkReturn BOOL SetPort(__in LPCSTR szPort) noexcept
	{
		BOOL bRet = SetVersion(1);
		if (bRet)
	...
	// Version 0 & Version 1 attribute.
	__checkReturn BOOL SetSecure(__in BOOL bSecure) noexcept
	{
		BOOL bRet = FALSE;
		LPCSTR szKey = "secure";
		bRet = SetVersion(1);
	...
	// Version 1 attribute.
	__checkReturn BOOL SetVersion(__in UINT nVersion) noexcept
	{
		BOOL bRet = FALSE;
		CHAR buff[20];
	...
	// Version 0 attribute.
	__checkReturn BOOL SetExpires(__in LPCSTR szExpires) noexcept
	{
		return AddAttribute("expires", szExpires);
	}

	__checkReturn BOOL SetExpires(__in const SYSTEMTIME &st) noexcept
	{
		_ATLTRY
		{
...
private:
	CCookie& Copy(__in const CCookie& thatCookie)
	{
	...
	void CopyToCString(__out CStringA& string, __in_ecount(pEnd-pStart) LPCSTR pStart, __in LPCSTR pEnd)
	{
...
public:
	...
	BOOL SetSessionID(LPCSTR szSessionID) noexcept
	{
...
public:
	...
	inline void SetShared(__in bool bShared) noexcept
	{
		m_bShared = bShared;
	}
	...
	CPair* Lookup( __in KINARGTYPE key ) noexcept
	{
#ifdef ATL_HTTP_PARAM_MULTIMAP
		POSITION pos = m_map.FindFirstWithKey(key);
		if (pos != NULL)
	...
	// iteration wrappers
	void GetNextAssoc( __inout POSITION& pos, __out KOUTARGTYPE key, __out VOUTARGTYPE value ) const
	{
	...
	CPair* GetNext( __inout POSITION& pos ) noexcept
	{
		return m_map.GetNext(pos);
	}
	...
	V& GetNextValue( __inout POSITION& pos ) noexcept
	{
		return m_map.GetNextValue(pos);
	}

	void GetAt( __in POSITION pos, __out KOUTARGTYPE key, __out VOUTARGTYPE value ) const
	{
		return m_map.GetAt(pos, key, value);
	}

	CPair* GetAt( __in POSITION pos ) noexcept
	{
		return m_map.GetAt(pos);
	}
	...
	V& GetValueAt( __in POSITION pos ) noexcept
	{
		return m_map.GetValueAt(pos);
	}

	// modification wrappers
	POSITION SetAt( __in KINARGTYPE key, __in_opt VINARGTYPE value )
	{
#ifdef ATL_HTTP_PARAM_MULTIMAP
	...
	bool RemoveKey( __in KINARGTYPE key ) noexcept
	{
#ifdef ATL_HTTP_PARAM_MULTIMAP
...
public:
	...
	LPCSTR Lookup(__in LPCSTR szName) const noexcept
	{
		_ATLTRY
		{
			if (!szName)
	...
	ATL_NOINLINE __checkReturn BOOL Parse(__inout LPSTR szQueryString) noexcept
	{
...
public:
	...
	CMultiPartFormParser(__in IHttpServerContext* pServerContext) noexcept :
		m_pCurrent(NULL),
		m_pEnd(NULL),
		m_pStart(NULL),
		m_dwBoundaryLen(0),
	...
	ATL_NOINLINE BOOL GetMultiPartData(
		...
		__in DWORD dwFlags=ATL_FORM_FLAG_NONE) noexcept
	{
		_ATLTRY
		{
			if (!InitializeParser())
...
private:
	...
	inline char AtlCharLower(__in char ch) noexcept
	{
		if (ch > 64 && ch < 91)
		{
			return ch+32;
	...
	ATL_NOINLINE __checkReturn BOOL InitializeParser() noexcept
	{
	...
	inline __checkReturn BOOL MoveToData() noexcept
	{
	...
	inline __checkReturn BOOL GetMimeData(__inout CStringA &str, __in_ecount(dwFieldLen) LPCSTR szField, __in DWORD dwFieldLen, __out LPBOOL pbFound, __in BOOL bIgnoreCase = FALSE) noexcept
	{
		_ATLTRY
		{
	...
	ATL_NOINLINE __checkReturn BOOL ReadUntilBoundary(__inout_opt CStringA* pStrData=NULL, __inout_opt CAtlTemporaryFile* pCtf=NULL) noexcept
	{
		_ATLTRY
		{
	...
	static inline __checkReturn BOOL IsStandardBoundaryChar(__in CHAR ch) noexcept
	{
		if ( (ch >= 'A' && ch <= 'Z') ||
			 (ch >= 'a' && ch <= 'z') ||
			 (ch >= '0' && ch <= '9') ||
	...
	inline __checkReturn BOOL IsValidTokenChar(__in CHAR ch) noexcept
	{
...
private:
	// Prevents copying.
	CMultiPartFormParser(__in const CMultiPartFormParser& /*that*/) noexcept
	{
		ATLASSERT(FALSE);
	}

	const CMultiPartFormParser& operator=(__in const CMultiPartFormParser& /*that*/) noexcept
	{
		ATLASSERT(FALSE);
		return (*this);
	}
...
protected:
	...
	void ClearFilesAndCookies() noexcept
	{
		m_Files.RemoveAll();
...
public:
	...
	const CHttpRequestParams& GetQueryParams() const noexcept
	{
	...
	// or from the body of the request for a POST request.
	const CHttpRequestParams& GetFormVars() const noexcept
	{
		return *m_pFormVars;
	}

	CHttpRequest() noexcept
	{
		m_bMultiPart = FALSE;
		m_dwBytesRead = 0;
		m_pFormVars = &m_QueryParams;
	...
	CHttpRequest(
		...
		__in DWORD dwFlags=ATL_FORM_FLAG_NONE)
		:m_pFormVars(NULL)
	{
		Construct();
	...
	CHttpRequest(__in IHttpRequestLookup *pRequestLookup)
		:m_pFormVars(NULL)
	{
	...
	__success(return != NULL) __checkReturn POSITION GetFirstCookie(__deref_out LPCSTR *ppszName, __deref_out const CCookie **ppCookie) noexcept
	{
		ATLASSERT(ppszName != NULL);
	...
	__success(return != NULL) __checkReturn POSITION GetNextCookie(__in POSITION pos, __deref_out LPCSTR *ppszName, __deref_out const CCookie **ppCookie) noexcept
	{
		ATLASSERT(pos != NULL);
		ATLASSERT(ppszName != NULL);
		ATLASSERT(ppCookie != NULL);
	...
	void SetServerContext(__in IHttpServerContext *pServerContext) noexcept
	{
		m_spServerContext = pServerContext;
	}

	BOOL Initialize(__in IHttpRequestLookup *pRequestLookup) noexcept
	{
		_ATLTRY
		{	
	...
	ATL_NOINLINE BOOL InitFromPost() noexcept
	{
		_ATLTRY
		{
			ATLASSUME(m_spServerContext != NULL);
	...
	// Equivalent to EXTENSION_CONTROL_BLOCK::cbAvailable.
	__checkReturn DWORD GetAvailableBytes()
	{
	...
	// Equivalent to the CONTENT_LENGTH server variable or EXTENSION_CONTROL_BLOCK::cbTotalBytes. 
	__checkReturn DWORD GetTotalBytes()
	{
		ATLENSURE(m_spServerContext);
	...
	// The size of the buffer can be determined by calling GetAvailableBytes.
	// Equivalent to EXTENSION_CONTROL_BLOCK::lpbData
	__checkReturn LPBYTE GetAvailableData()
	{
		ATLENSURE(m_spServerContext);
	...
	// This is the part of the URL that appears after the question mark (?). 
	// Equivalent to the QUERY_STRING server variable or EXTENSION_CONTROL_BLOCK::lpszQueryString.
	__checkReturn LPCSTR GetQueryString()
	{
		ATLENSURE(m_spServerContext);
	...
	// Examples of common HTTP methods include "GET" and "POST".
	// Equivalent to the REQUEST_METHOD server variable or EXTENSION_CONTROL_BLOCK::lpszMethod.
	__checkReturn LPCSTR GetMethodString()
	{
		ATLENSURE(m_spServerContext);
	...
	//      LINK
	//      UNLINK
	__checkReturn HTTP_METHOD GetMethod()
	{
	...
	// On entry, pdwSize should point to a DWORD that indicates the size of the buffer in bytes.
	// On exit, the DWORD contains the number of bytes transferred or available to be transferred into the buffer (including the nul-terminating byte).
	__checkReturn BOOL GetAuthUserName(__out_ecount_part(*pdwSize,*pdwSize) LPSTR szBuff, __inout DWORD *pdwSize)
	{
		ATLENSURE(m_spServerContext);
	...
	// On entry, pdwSize should point to a DWORD that indicates the size of the buffer in bytes.
	// On exit, the DWORD contains the number of bytes transferred or available to be transferred into the buffer (including the nul-terminating byte).
	__checkReturn BOOL GetPhysicalPath(__out_ecount_part(*pdwSize,*pdwSize) LPSTR szBuff, __inout DWORD *pdwSize)
	{
		ATLENSURE(m_spServerContext);
	...
	// On entry, pdwSize should point to a DWORD that indicates the size of the buffer in bytes.
	// On exit, the DWORD contains the number of bytes transferred or available to be transferred into the buffer (including the nul-terminating byte).
	__checkReturn BOOL GetAuthUserPassword(__out_ecount_part(*pdwSize,*pdwSize) LPSTR szBuff, __inout DWORD *pdwSize)
	{
		ATLENSURE(m_spServerContext);
	...
	// On entry, pdwSize should point to a DWORD that indicates the size of the buffer in bytes.
	// On exit, the DWORD contains the number of bytes transferred or available to be transferred into the buffer (including the nul-terminating byte).
	__checkReturn BOOL GetUrl(__out_ecount_part(*pdwSize,*pdwSize) LPSTR szBuff, DWORD *pdwSize)
	{
		ATLENSURE(m_spServerContext);
	...
	// On entry, pdwSize should point to a DWORD that indicates the size of the buffer in bytes.
	// On exit, the DWORD contains the number of bytes transferred or available to be transferred into the buffer (including the nul-terminating byte).
	__checkReturn BOOL GetUserHostName(__out_ecount_part(*pdwSize,*pdwSize) LPSTR szBuff, __inout DWORD *pdwSize)
	{
		ATLENSURE(m_spServerContext);
	...
	// On entry, pdwSize should point to a DWORD that indicates the size of the buffer in bytes.
	// On exit, the DWORD contains the number of bytes transferred or available to be transferred into the buffer (including the nul-terminating byte).
	__checkReturn BOOL GetUserHostAddress(__out_ecount_part(*pdwSize,*pdwSize) LPSTR szBuff, __inout DWORD *pdwSize)
	{
		ATLENSURE(m_spServerContext);
	...
	// For example, if GetPathTranslated returns "c:\inetpub\vcisapi\hello.srf\goodmorning",
	// then this function returns "c:\inetpub\vcisapi\hello.srf".
	__checkReturn LPCSTR GetScriptPathTranslated()
	{
		ATLENSURE(m_spServerContext);
	...
	// Equivalent to the PATH_TRANSLATED server variable or EXTENSION_CONTROL_BLOCK::lpszPathTranslated.
	__checkReturn LPCSTR GetPathTranslated()
	{
		ATLENSURE(m_spServerContext);
	...
	// This is the part of the URL that appears after the server name, but before the query string.
	// Equivalent to the PATH_INFO server variable or EXTENSION_CONTROL_BLOCK::lpszPathInfo.
	__checkReturn LPCSTR GetPathInfo()
	{
		ATLENSURE(m_spServerContext);
	...
	__checkReturn BOOL GetAuthenticated()
	{
	...
	// On entry, pdwSize should point to a DWORD that indicates the size of the buffer in bytes.
	// On exit, the DWORD contains the number of bytes transferred or available to be transferred into the buffer (including the nul-terminating byte).
	__checkReturn BOOL GetAuthenticationType(__out_ecount_part(*pdwSize,*pdwSize) LPSTR szBuff, __inout DWORD *pdwSize)
	{
		ATLENSURE(m_spServerContext);
	...
	// On entry, pdwSize should point to a DWORD that indicates the size of the buffer in bytes.
	// On exit, the DWORD contains the number of bytes transferred or available to be transferred into the buffer (including the nul-terminating byte).
	__checkReturn BOOL GetUserName(__out_ecount_part(*pdwSize,*pdwSize) LPSTR szBuff, DWORD __inout *pdwSize)
	{
		ATLENSURE(m_spServerContext);
	...
	// On entry, pdwSize should point to a DWORD that indicates the size of the buffer in bytes.
	// On exit, the DWORD contains the number of bytes transferred or available to be transferred into the buffer (including the nul-terminating byte).
	 __checkReturn BOOL GetUserAgent(__out_ecount_part(*pdwSize,*pdwSize) LPSTR szBuff, __inout DWORD *pdwSize)
	 {
		ATLENSURE(m_spServerContext);
	...
	// On entry, pdwSize should point to a DWORD that indicates the size of the buffer in bytes.
	// On exit, the DWORD contains the number of bytes transferred or available to be transferred into the buffer (including the nul-terminating byte).
	 __checkReturn BOOL GetUserLanguages(__out_ecount_part(*pdwSize,*pdwSize) LPSTR szBuff, __inout DWORD *pdwSize)
	 {
		ATLENSURE(m_spServerContext);
	...
	// On entry, pdwSize should point to a DWORD that indicates the size of the buffer in bytes.
	// On exit, the DWORD contains the number of bytes transferred or available to be transferred into the buffer (including the nul-terminating byte).
	__checkReturn BOOL GetAcceptTypes(__out_ecount_part(*pdwSize,*pdwSize) LPSTR szBuff, __inout DWORD *pdwSize)
	{
		ATLENSURE(m_spServerContext);
	...
	// On entry, pdwSize should point to a DWORD that indicates the size of the buffer in bytes.
	// On exit, the DWORD contains the number of bytes transferred or available to be transferred into the buffer (including the nul-terminating byte).
	__checkReturn BOOL GetAcceptEncodings(__out_ecount_part(*pdwSize,*pdwSize) LPSTR szBuff, __inout DWORD *pdwSize)
	{
		ATLENSURE(m_spServerContext);
	...
	// On entry, pdwSize should point to a DWORD that indicates the size of the buffer in bytes.
	// On exit, the DWORD contains the number of bytes transferred or available to be transferred into the buffer (including the nul-terminating byte).
	__checkReturn BOOL GetUrlReferer(__out_ecount_part(*pdwSize,*pdwSize) LPSTR szBuff, __inout DWORD *pdwSize)
	{
		ATLENSURE(m_spServerContext);
	...
	// On entry, pdwSize should point to a DWORD that indicates the size of the buffer in bytes.
	// On exit, the DWORD contains the number of bytes transferred or available to be transferred into the buffer (including the nul-terminating byte).
	__checkReturn BOOL GetScriptName(__out_ecount_part(*pdwSize,*pdwSize) LPSTR szBuff, __inout DWORD *pdwSize)
	{
		ATLENSURE(m_spServerContext);
	...
	// from the browser.
	__checkReturn BOOL GetCookies(__out_ecount_part(*pdwSize,*pdwSize) LPSTR szBuf, __inout LPDWORD pdwSize) const
	{		
	...
	// from the browser.
	__checkReturn BOOL GetCookies(__inout CStringA& strBuff) const noexcept
	{
	...
	ATL_NOINLINE const CCookie& Cookies(__in LPCSTR szName)
	{
		if (GetRequestCookies())
		{
	...
	// Returns TRUE on success, FALSE on failure. Call GetLastError to get extended error information.
	__checkReturn BOOL GetPhysicalPath(__out CStringA &str) noexcept
	{
	...
	// Returns TRUE on success, FALSE on failure. Call GetLastError to get extended error information.
	__checkReturn BOOL GetUserHostName(__out CStringA &str) noexcept
	{
	...
	// Returns TRUE on success, FALSE on failure. Call GetLastError to get extended error information.
	__checkReturn BOOL GetUserHostAddress(__out CStringA &str) noexcept
	{
	...
	// Returns TRUE on success, FALSE on failure. Call GetLastError to get extended error information.
	__checkReturn BOOL GetAuthenticationType(__out CStringA &str) noexcept
	{
	...
	// Returns TRUE on success, FALSE on failure. Call GetLastError to get extended error information.
	__checkReturn BOOL GetUserName(__out CStringA &str) noexcept
	{
	...
	// Returns TRUE on success, FALSE on failure. Call GetLastError to get extended error information.
	__checkReturn BOOL GetUserAgent(__out CStringA &str) noexcept
	{
	...
	// Returns TRUE on success, FALSE on failure. Call GetLastError to get extended error information.
	__checkReturn BOOL GetUserLanguages(__out CStringA &str) noexcept
	{
	...
	// Returns TRUE on success, FALSE on failure. Call GetLastError to get extended error information.
	__checkReturn BOOL GetAuthUserName(__out CStringA &str) noexcept
	{
	...
	// Returns TRUE on success, FALSE on failure. Call GetLastError to get extended error information.
	__checkReturn BOOL GetAuthUserPassword(__out CStringA &str) noexcept
	{
	...
	// Returns TRUE on success, FALSE on failure. Call GetLastError to get extended error information.
	__checkReturn BOOL GetUrl(__out CStringA &str) noexcept
	{
	...
	// Returns TRUE on success, FALSE on failure. Call GetLastError to get extended error information.
	__checkReturn BOOL GetAcceptTypes(__out CStringA &str) noexcept
	{
	...
	// Returns TRUE on success, FALSE on failure. Call GetLastError to get extended error information.
	__checkReturn BOOL GetAcceptEncodings(__out CStringA& str) noexcept
	{
	...
	// Returns TRUE on success, FALSE on failure. Call GetLastError to get extended error information.
	__checkReturn BOOL GetUrlReferer(__out CStringA &str) noexcept
	{
	...
	// Returns TRUE on success, FALSE on failure. Call GetLastError to get extended error information.
	__checkReturn BOOL GetScriptName(__out CStringA &str) noexcept
	{
	...
	// Returns TRUE on success, FALSE on failure.
	__checkReturn BOOL GetRequestCookies() noexcept
	{
		BOOL bRet = FALSE;
...
private:
	// Call this function to copy a substring to a CString reference and ensure nul-termination.
	inline void CopyToCString(__out CStringA& string, __in_ecount(pEnd-pStart) LPCSTR pStart, __in LPCSTR pEnd)
	{
		ATLENSURE( pStart != NULL );
		ATLENSURE( pEnd != NULL );
	...
	inline LPCSTR SkipSpace(__in LPCSTR sz, __in WORD nCodePage) noexcept
	{
		if (sz == NULL)
			return NULL;
...
public:
	CWriteStreamHelper() noexcept
		:m_pStream(NULL)
	{
		m_ulACP = _AtlGetConversionACP();
	}

	CWriteStreamHelper(__in_opt IWriteStream *pStream) noexcept
	{
		m_pStream = pStream;
		m_ulACP = _AtlGetConversionACP();
	}
	...
	IWriteStream *Attach(__in IWriteStream *pStream) noexcept
	{
	...
	// Returns TRUE on success, FALSE on failure.
	BOOL Write(__in_z LPCSTR szOut) noexcept
	{
	...
	// Call this function to write data to the IWriteStream interface managed by this object.
	// Returns TRUE on success, FALSE on failure.
	BOOL Write(__in int n) noexcept
	{
		CHAR szTmp[21];
	...
	// Call this function to write data to the IWriteStream interface managed by this object.
	// Returns TRUE on success, FALSE on failure.
	BOOL Write(__in unsigned int u) noexcept
	{
		CHAR szTmp[21];
	...
	// Call this function to write data to the IWriteStream interface managed by this object.
	// Returns TRUE on success, FALSE on failure.
	BOOL Write(__in short int w) noexcept
	{
		return Write((int) w);
	}
	...
	// Call this function to write data to the IWriteStream interface managed by this object.
	// Returns TRUE on success, FALSE on failure.
	BOOL Write(__in unsigned short int w) noexcept
	{
	...
	// Call this function to write data to the IWriteStream interface managed by this object.
	// Returns TRUE on success, FALSE on failure.
	BOOL Write(__in long int dw) noexcept
	{
		CHAR szTmp[21];
	...
	// Call this function to write data to the IWriteStream interface managed by this object.
	// Returns TRUE on success, FALSE on failure.
	BOOL Write(__in unsigned long int dw) noexcept
	{
		CHAR szTmp[21];
	...
	// Call this function to write data to the IWriteStream interface managed by this object.
	// Returns TRUE on success, FALSE on failure.
	BOOL Write(__in double d, __in int nDigitCount=ATL_DEFAULT_PRECISION) noexcept
	{
		ATLASSUME(m_pStream != NULL);
	...
	// Call this function to write data to the IWriteStream interface managed by this object.
	// Returns TRUE on success, FALSE on failure.
	BOOL Write(__in __int64 i) noexcept
	{
		CHAR szTmp[21];
	...
	// Call this function to write data to the IWriteStream interface managed by this object.
	// Returns TRUE on success, FALSE on failure.
	BOOL Write(__in unsigned __int64 i) noexcept
	{
		CHAR szTmp[21];
	...
	// Call this function to write data to the IWriteStream interface managed by this object.
	// Returns TRUE on success, FALSE on failure.
	BOOL Write(__in CURRENCY c) noexcept
	{
		CHAR szDest[256];
		CHAR szNumber[32];
	...
	// Call this function to write data to the IWriteStream interface managed by this object.
	// Returns TRUE on success, FALSE on failure.
	BOOL Write(__in LPCWSTR wsz) noexcept
	{
		ATLASSUME(m_pStream != NULL);
	...
	CWriteStreamHelper& operator<<(__in LPCSTR szStr)
	{
		if (!Write(szStr))
			AtlThrow(E_FAIL);
		return *this;
	...
	// Use this operator to write data to the IWriteStream interface managed by this object.
	CWriteStreamHelper& operator<<(__in LPCWSTR wszStr)
	{
		if (!Write(wszStr))
			AtlThrow(E_FAIL);
		return *this;
	...
	// Use this operator to write data to the IWriteStream interface managed by this object.
	CWriteStreamHelper& operator<<(__in int n)
	{
		if (!Write(n))
			AtlThrow(E_FAIL);
		return *this;
	...
	// Use this operator to write data to the IWriteStream interface managed by this object.
	CWriteStreamHelper& operator<<(__in short int w)
	{
		if (!Write(w))
			AtlThrow(E_FAIL);
		return *this;
	...
	// Use this operator to write data to the IWriteStream interface managed by this object.
	CWriteStreamHelper& operator<<(__in unsigned int u)
	{
		if (!Write(u))
			AtlThrow(E_FAIL);
		return *this;
	...
	// Use this operator to write data to the IWriteStream interface managed by this object.
	CWriteStreamHelper& operator<<(__in long int dw)
	{
		if (!Write(dw))
			AtlThrow(E_FAIL);
		return *this;
	...
	// Use this operator to write data to the IWriteStream interface managed by this object.
	CWriteStreamHelper& operator<<(__in unsigned long int dw)
	{
		if (!Write(dw))
			AtlThrow(E_FAIL);
		return *this;
	...
	// Use this operator to write data to the IWriteStream interface managed by this object.
	CWriteStreamHelper& operator<<(__in double d)
	{
		if (!Write(d))
			AtlThrow(E_FAIL);
		return *this;
	...
	// Use this operator to write data to the IWriteStream interface managed by this object.
	CWriteStreamHelper& operator<<(__in __int64 i)
	{
		if (!Write(i))
			AtlThrow(E_FAIL);
		return *this;
	...
	// Use this operator to write data to the IWriteStream interface managed by this object.
	CWriteStreamHelper& operator<<(__in unsigned __int64 i)
	{
		if (!Write(i))
			AtlThrow(E_FAIL);
		return *this;
	...
	// Use this operator to write data to the IWriteStream interface managed by this object.
	CWriteStreamHelper& operator<<(__in CURRENCY c)
	{
		if (!Write(c))
			AtlThrow(E_FAIL);
		return *this;
...
public:
	...
	BOOL Alloc(__in DWORD dwSize) noexcept
	{
		if (m_dwAlloc >= dwSize)
		{
			return TRUE;
	...
	BOOL ReAlloc(__in DWORD dwNewSize) noexcept
	{
	...
	BOOL Append(__in LPCSTR sz, __in int nLen = -1) noexcept
	{
		if (!sz)
			return FALSE;
...
public:
	...
	virtual ~CHttpResponse() noexcept
	{
		Flush(TRUE);
	...
	// Returns TRUE on success, FALSE on failure.
	__checkReturn BOOL Initialize(__in IHttpServerContext *pServerContext) noexcept
	{
		ATLASSERT(pServerContext != NULL);
		if (!pServerContext)
			return FALSE;
	...
	__checkReturn BOOL Initialize(__in IHttpRequestLookup *pLookup)
	{
		ATLASSERT(pLookup);
		if (!pLookup)
			return FALSE;
	...
	// Returns a pointer to the IHttpServerContext interface for the current request.
	__checkReturn HRESULT GetServerContext(__deref_out_opt IHttpServerContext ** ppOut) noexcept
	{
		return m_spServerContext.CopyTo(ppOut);
	}
	...
	void SetBufferOutput(__in BOOL bBufferOutput, __in DWORD dwBufferLimit=ATL_ISAPI_BUFFER_SIZE) noexcept
	{
	...
	__checkReturn BOOL GetBufferOutput() noexcept
	{
	...
	__checkReturn BOOL HaveSentHeaders() noexcept
	{
	...
	void HaveSentHeaders(__in BOOL bSent) noexcept
	{
	...
	// the entire buffer is flushed.
	// See GetBufferLimit.
	void SetBufferLimit(__in DWORD dwBufferLimit) noexcept
	{
		if (m_bBufferOutput)
		{
	...
	BOOL SetContentType(__in_opt LPCSTR szContentType) noexcept
	{
		_ATLTRY
		{
	...
	// See GetStatusCode.
	void SetStatusCode(__in int nCode) noexcept
	{
		m_nStatusCode = nCode;
	}
	...
	BOOL SetCacheControl(__in_opt LPCSTR szCacheControl) noexcept
	{
		_ATLTRY
		{
	...
	BOOL SetExpiresAbsolute(__in const SYSTEMTIME& stExpires) noexcept
	{
		_ATLTRY
		{
			CStringA strExpires;
	...
	BOOL SetExpires(__in long lMinutes) noexcept
	{
		CFileTime ft;
	...
	BOOL SetWriteToClient(__in BOOL bSendOutput) noexcept
	{
	...
	BOOL GetWriteToClient() noexcept
	{
	...
	__checkReturn BOOL WriteLen(__in_ecount(dwLen) LPCSTR szOut, __in DWORD dwLen) noexcept
	{
		ATLASSUME(m_spServerContext != NULL);
		if (!szOut)
			return FALSE;
	...
	BOOL Redirect(__in LPCSTR szUrl, __in HTTP_REDIRECT statusCode=HTTP_REDIRECT_MOVED, __in BOOL bSendBody=TRUE)
	{
		CStringA strBody;
		LPCSTR szBody = NULL;
		if (bSendBody &&
	...
	//
	// See RFC 2616 section 10.3 for more information on redirection.
	BOOL Redirect(__in LPCSTR szUrl, __in LPCSTR szBody, __in HTTP_REDIRECT statusCode=HTTP_REDIRECT_MOVED) noexcept
	{
	...
	BOOL AppendHeader(__in LPCSTR szName, __in_opt LPCSTR szValue) noexcept
	{
		ATLASSERT(szName);
		BOOL bRet = FALSE;
		_ATLTRY
	...
	BOOL AppendCookie(__in const CCookie& cookie) noexcept
	{
	...
	BOOL AppendCookie(__in LPCSTR szName, __in_opt LPCSTR szValue) noexcept
	{
		ATLASSERT(szName);
		BOOL bRet = FALSE;
		_ATLTRY
	...
	BOOL DeleteCookie(__in LPCSTR szName) noexcept
	{
		ATLASSERT(szName);
		BOOL bRet = FALSE;
		_ATLTRY
	...
	void RenderHeaders(CStringA& strHeaders) noexcept
	{
		_ATLTRY
		{
	...
	// Any headers that have been set in the response will be sent just before the 
	// data is written to the client if no headers have been sent up to that point.
	BOOL Flush(BOOL bFinal=FALSE) noexcept
	{
	...
	BOOL AsyncPrep(__in BOOL fKeepConn=FALSE) noexcept
	{
		ATLASSUME(m_spServerContext != NULL);
	...
	BOOL TransmitFile(__in HANDLE hFile, __in LPCSTR szContentType="text/plain") noexcept
	{
		ATLASSUME(m_spServerContext != NULL);
...
public:
	...
	void RequestHandled(__in AtlServerRequest *pRequestInfo, __in BOOL bSuccess) noexcept
	{
		if (m_pPerfObjectInstance != NULL)
...
public:
	...
	void RequestHandled(AtlServerRequest * /*pRequestInfo*/, BOOL /*bSuccess*/) noexcept
	{
	}
...
public:
	...
	{
		DllInfo& operator=(__in const DllInfo& right) noexcept
		{
			if (this != &right)
			{
	...
	BOOL Add(__in HINSTANCE hInst, __out DllInfo *pInfo)
	{
	...
	void Remove(HINSTANCE /*hInst*/, __in DllInfo *pInfo)
	{
		ATLENSURE(pInfo!=NULL);
...
inline bool operator==(__in const CDllCachePeer::DllInfo& left, __in const CDllCachePeer::DllInfo& right) noexcept
{
...
class _CComObjectHeapNoLock : public Base
{
	...
	STDMETHOD_(ULONG, AddRef)() noexcept {return InternalAddRef();}
	STDMETHOD_(ULONG, Release)() noexcept
	{
		ULONG l = InternalRelease();
		if (l == 0)
	...
	static HRESULT WINAPI CreateInstance(_CComObjectHeapNoLock<Base>** pp, HANDLE hHeap) noexcept;	
};



template <class Base>
HRESULT WINAPI _CComObjectHeapNoLock<Base>::CreateInstance(__deref_out _CComObjectHeapNoLock<Base>** pp, __in HANDLE hHeap) noexcept
{
	ATLASSERT(pp != NULL);
	if (pp == NULL)
		return E_POINTER;
...
inline CServerContext* CreateServerContext(__in HANDLE hRequestHeap) noexcept
{
...
public:
	...
	CTransferServerContext() noexcept
	{
		m_pStream = NULL;
	}
	...
	BOOL Initialize(__in LPCSTR szUrl, __in IWriteStream *pStream, __in IHttpServerContext *pParent) noexcept
	{
		m_pStream = pStream;
		m_spParent = pParent;
...
protected:
	...
	{
	public:
		static bool IsEqual(__in const ServiceNode& t1, __in const ServiceNode& t2) noexcept
		{
...
public:
	...
	DWORD HttpExtensionProc(LPEXTENSION_CONTROL_BLOCK lpECB) noexcept
	{
	...
	virtual LPCSTR GetExtensionDesc() noexcept { return "VC Server Classes"; }
	virtual int GetNumPoolThreads() noexcept { return 0; }
	virtual int GetPoolStackSize() noexcept { return 0; }
	virtual HANDLE GetIOCompletionHandle() noexcept { return INVALID_HANDLE_VALUE; }
	virtual DWORD GetDllCacheTimeout() noexcept { return ATL_DLL_CACHE_TIMEOUT; }
	virtual DWORD GetStencilCacheTimeout() noexcept { return ATL_STENCIL_CACHE_TIMEOUT; }
	virtual LONGLONG GetStencilLifespan() noexcept { return ATL_STENCIL_LIFESPAN; }
	...
	BOOL SetCriticalIsapiError(__in DWORD dwErr = 1) noexcept
	{
	...
	DWORD GetCriticalIsapiError() noexcept
	{
	...
	BOOL GetExtensionVersion(__out HSE_VERSION_INFO* pVer) noexcept
	{
		ATLASSERT(pVer!=NULL);
		if(pVer==NULL)
		{
	...
	BOOL TerminateExtension(DWORD /*dwFlags*/) noexcept
	{
		m_critSec.Lock();
	...
	void HandleError(__in IHttpServerContext *pServerContext, __in DWORD dwStatus, __in DWORD dwSubStatus) noexcept
	{
	...
	HTTP_CODE GetHandlerName(__in LPCSTR szFileName, __out_ecount(MAX_PATH+ATL_MAX_HANDLER_NAME+2) LPSTR szHandlerName) noexcept
	{
	...
	BOOL ProcessDebug(__inout AtlServerRequest *pRequestInfo)
{
	...
	__success(SUCCEEDED(return)) __checkReturn HRESULT GetService(__in REFGUID guidService, __in REFIID riid, __deref_out void **ppvObject) noexcept
	{
		if (!ppvObject)
			return E_POINTER;
...
{ \
extern "C" ATL_NOINLINE inline BOOL __declspec(dllexport) __stdcall GetAtlHandlerByName(LPCSTR szHandlerName, IIsapiExtension *pExtension, IUnknown **ppHandler) noexcept \
{ \
	*ppHandler = NULL; \
...
extern "C" ATL_NOINLINE inline  BOOL __declspec(dllexport) __stdcall InitializeAtlHandlers(IHttpServerContext *pContext, IIsapiExtension *pExt) noexcept \
{ \
	ATL::_HANDLER_ENTRY **pEntry = &__phdlrA; \
	BOOL bRet = TRUE; \
	while (pEntry != &__phdlrZ) \
...
extern "C" ATL_NOINLINE inline void __declspec(dllexport) __stdcall UninitializeAtlHandlers() noexcept \
{\
	ATL::_HANDLER_ENTRY **pEntry = &__phdlrA; \
	while (pEntry != &__phdlrZ) \
	{ \
...
public:
	...
	virtual bool CheckAccount(IHttpServerContext *pContext, const SID *psidAuthGroup) noexcept
	{
		(pContext); // unused
	...
	HTTP_CODE HandleError(AtlServerRequest *pRequestInfo) noexcept
	{
		pRequestInfo; // unused
		return HTTP_FAIL;
	}

	__checkReturn bool CheckAuthAccount(__in IHttpServerContext *pContext, __in const SID* psidAuthGroup) noexcept
	{
		ATLASSERT(pContext);
		ATLASSERT(psidAuthGroup);
...
{
public:
	virtual bool CheckAccount(__in IHttpServerContext *pContext, __in const SID* psidAuthGroup) noexcept
	{
		return CheckAuthAccount(pContext, psidAuthGroup);
	}

	HTTP_CODE HandleError(__in AtlServerRequest *pRequestInfo) noexcept
	{
