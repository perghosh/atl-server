...
inline CPerfMon::CounterInfo* CPerfMon::CategoryInfo::_GetCounterInfo(UINT nIndex) noexcept
{
	ATLASSERT(nIndex < _GetNumCounters());
	return &m_counters[nIndex];
}

inline UINT CPerfMon::CategoryInfo::_GetNumCounters() noexcept
{
	return (UINT) m_counters.GetCount();
}

inline CPerfMon::~CPerfMon() noexcept
{
	UnInitialize();
}

inline HRESULT CPerfMon::CreateMap(LANGID language, HINSTANCE hResInstance, UINT* pSampleRes) noexcept
{
	(language); // unused
	(hResInstance); // unused
	(pSampleRes); // unused
...
inline UINT CPerfMon::_GetNumCategoriesAndCounters() noexcept
{
...
inline CPerfMon::CategoryInfo* CPerfMon::_GetCategoryInfo(UINT nIndex) noexcept
{
	ATLASSERT(nIndex < _GetNumCategories());
	return &m_categories[nIndex];
}

inline UINT CPerfMon::_GetNumCategories() noexcept
{
	return (UINT) m_categories.GetCount();
}

inline CPerfObject* CPerfMon::_GetFirstInstance(CAtlFileMappingBase* pBlock) noexcept
{
...
inline CPerfObject* CPerfMon::_GetNextInstance(CPerfObject* pInstance) noexcept
{
...
inline CAtlFileMappingBase* CPerfMon::_GetNextBlock(CAtlFileMappingBase* pBlock) noexcept
{
...
inline CAtlFileMappingBase* CPerfMon::_OpenNextBlock(CAtlFileMappingBase* pPrev) noexcept
{
...
inline CAtlFileMappingBase* CPerfMon::_AllocNewBlock(CAtlFileMappingBase* pPrev, BOOL* pbExisted /* == NULL */) noexcept
{
	CAtlFileMappingBase* pMem = NULL;
	_ATLTRY
	{
...
inline HRESULT CPerfMon::_OpenAllBlocks() noexcept
{
	HRESULT hr;
...
inline HRESULT CPerfMon::_LoadMap(DWORD* pData) noexcept
{
	_ATLTRY
	{
		HRESULT hr;
...
inline HRESULT CPerfMon::_SaveMap() noexcept
{
	_ATLTRY
	{
...
inline CPerfMon::CategoryInfo* CPerfMon::_FindCategoryInfo(DWORD dwCategoryId) noexcept
{
	for (UINT i=0; i<_GetNumCategories(); i++)
	{
		CategoryInfo* pCategoryInfo = _GetCategoryInfo(i);
...
inline CPerfMon::CounterInfo* CPerfMon::_FindCounterInfo(CategoryInfo* pCategoryInfo, DWORD dwCounterId) noexcept
{
...
inline CPerfMon::CounterInfo* CPerfMon::_FindCounterInfo(DWORD dwCategoryId, DWORD dwCounterId) noexcept
{
...
inline BOOL CPerfMon::_WantCategoryType(__in_z LPWSTR szValue, __in DWORD dwCategoryId)
{
...
inline LPBYTE CPerfMon::_AllocData(LPBYTE& pData, ULONG nBytesAvail, ULONG* pnBytesUsed, size_t nBytesNeeded) noexcept
{
...
inline DWORD* CPerfMon::_GetBlockId_NoThrow(CAtlFileMappingBase* pBlock) noexcept
{
	if (pBlock == NULL)
		return NULL;
...
inline void CPerfMon::_FillCategoryType(CategoryInfo* pCategoryInfo) noexcept
{
...
inline void CPerfMon::_FillCounterDef(CounterInfo* pCounterInfo, ULONG* pnCounterBlockSize) noexcept
{
...
inline DWORD CPerfMon::Open(LPWSTR szDeviceNames) noexcept
{
...
inline DWORD CPerfMon::Close() noexcept
{
	UnInitialize();
	return ERROR_SUCCESS;
}
...
inline HRESULT CPerfMon::_ReserveStringRange(DWORD& dwFirstCounter, DWORD& dwFirstHelp) noexcept
{
	CRegKey rkApp;
	CString strAppKey;
	DWORD dwErr;
...
inline HRESULT CPerfMon::_UnregisterStrings() noexcept
{
	_ATLTRY
	{
		CString str;
...
inline HRESULT CPerfMon::Unregister() noexcept
{
	CString str;
	HRESULT hr;
	DWORD dwErr;
...
inline HRESULT CPerfMon::Initialize() noexcept
{
	CMutex tempLock;
	CString strAppName;
	HRESULT hr;
...
inline void CPerfMon::UnInitialize() noexcept
{
	if (m_lock.m_h != NULL)
		m_lock.Close();
	m_aMem.RemoveAll();
...
inline HRESULT CPerfMon::ReleaseInstance(CPerfObject* pInstance) noexcept
{
...
inline HRESULT CPerfMon::LockPerf(DWORD dwTimeout /* == INFINITE */) noexcept
{
	if (m_lock.m_h == NULL)
		return E_UNEXPECTED;
...
inline void CPerfMon::UnlockPerf() noexcept
{
	m_lock.Release();
}
...
inline HRESULT CPerfMon::AddCategoryDefinition(
	...
	BOOL bInstanceLess,
	UINT nStructSize,
	UINT nMaxInstanceNameLen) noexcept
{
...
inline void CPerfMon::ClearMap() noexcept
{
...
ATL_NOINLINE inline HRESULT CPerfMon::PersistToXML(IStream *pStream, BOOL bFirst/*=TRUE*/, BOOL bLast/*=TRUE*/)
{
	...
	//strXML.Format("\t<perfmon name=\"%s\">\r\n", CT2CA(GetAppName())); TODO: Fix this
   strXML.Format("\t<perfmon name=\"app name\">\r\n"); // replace this
	hr = pStream->Write(strXML, strXML.GetLength(), &nLen);
...
ATL_NOINLINE inline HRESULT CPerfMon::_GetAttribute(IXMLDOMNode *pNode, LPCWSTR szAttrName, BSTR *pbstrVal) noexcept
{
...
ATL_NOINLINE inline HRESULT RegisterPerfMon(HINSTANCE hDllInstance /* = _AtlBaseModule.GetModuleInstance() */) noexcept 
{
...
ATL_NOINLINE inline HRESULT UnregisterPerfMon() noexcept 
{ 
	CPerfMon **ppPerf = &__pperfA; 
	HRESULT hr = S_OK; 
	while (ppPerf != &__pperfZ) 
...
extern "C" ATL_NOINLINE inline DWORD __declspec(dllexport) WINAPI OpenPerfMon(LPWSTR lpDeviceNames) noexcept
{
	CPerfMon **ppPerf = &__pperfA;
	DWORD dwErr = 0;
	while (ppPerf != &__pperfZ)
...
extern "C" ATL_NOINLINE inline DWORD __declspec(dllexport) WINAPI ClosePerfMon() noexcept
{
	CPerfMon **ppPerf = &__pperfA;
	while (ppPerf != &__pperfZ)
	{
